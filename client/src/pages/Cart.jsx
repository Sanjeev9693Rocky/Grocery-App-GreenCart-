    ...
    const placeOrder = async (e) => {
        try {
            ...
            if (paymentOption === "COD") {
                ...
                const { data: order } = await axios.post('/api/order/stripe', {
                    userId: user._id,
                    items: cartArray.map(item => ({ product: item._id, quantity: item.quantity })),
                    address: selectedAddresses._id
                })
                try {
                    ...
                    var options = {
                        ...
                        handler: async function (response) {
                            const body = {
                                ...response, amount: order.amount, userId: user._id,
                                items: cartArray.map(item => ({ product: item._id, quantity: item.quantity })),
                                address: selectedAddresses._id
                            };

                            const { data } = await axios.post("/api/order/verifyOrder", body);
                            if (data.success) {
                                toast.success(data.message)
                                setCartItems({})
                                navigate('/my-orders')
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        <p className="text-gray-500">{selectedAddresses ? `${selectedAddresses.street},${selectedAddresses.village}, ${selectedAddresses.city}, ${selectedAddresses.state}, ${selectedAddresses.country}` : "No address found"}</p>
